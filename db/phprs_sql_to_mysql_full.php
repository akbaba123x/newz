...
phprs_sql_db_name -- Vrátí seznam všech databází
phprs_sql_db_query -- Pošle MySQL dotaz
...
phprs_sql_field_flags --  Načte příznaky sloupce tabulky
phprs_sql_field_len --  Vrací délku sloupce tabulky
phprs_sql_field_name --  Načte název sloupce tabulky
phprs_sql_field_seek --  Nastaví ukazatel na zadaný sloupec
phprs_sql_field_table --  Zjistí jméno tabulky, v níž se nachází uvedený sloupec
phprs_sql_field_type --  Vrací typ specifikovaného sloupce.
...
phprs_sql_list_dbs --  Načte všechny dostupné databáze na MySQL serveru
phprs_sql_list_fields -- Načte výsledek s obsahem sloupce
phprs_sql_list_processes -- List MySQL processes
phprs_sql_list_tables -- Načte všechny tabulky v MySQL databázi
...
phprs_sql_tablename -- Načte jméno tabulky
phprs_sql_unbuffered_query --  Pošle SQL dotaz bez načtení a bufferování výsledných záznamů
*/
...
function phprs_sql_db_name($vysledek,$zaznam,$pole = false)
{
if ($pole === false):
  return mysql_db_name($vysledek,$zaznam);
else:
  return mysql_db_name($vysledek,$zaznam,$pole);
endif;
}

function phprs_sql_db_query($databaze,$dotaz,$spojeni = false)
{
if ($spojeni === false):
  return mysql_db_query($databaze,$dotaz);
else:
  return mysql_db_query($databaze,$dotaz,$spojeni);
endif;
}
...
function phprs_sql_field_flags($vysledek,$cislo_sloupce)
{
return mysql_field_flags($vysledek,$cislo_sloupce);
}

function phprs_sql_field_len($vysledek,$cislo_sloupce)
{
return mysql_field_len($vysledek,$cislo_sloupce);
}

function phprs_sql_field_name($vysledek,$cislo_sloupce)
{
return mysql_field_name($vysledek,$cislo_sloupce);
}
...
function phprs_sql_field_table($vysledek,$cislo_sloupce)
{
return mysql_field_table($vysledek,$cislo_sloupce);
}

function phprs_sql_field_type($vysledek,$cislo_sloupce)
{
return mysql_field_type($vysledek,$cislo_sloupce);
}
...
function phprs_sql_list_dbs($spojeni = false)
{
if ($spojeni === false):
  return mysql_list_dbs();
else:
  return mysql_list_dbs($spojeni);
endif;
}

function phprs_sql_list_fields($jmeno_databaze,$jmeno_tabulky,$spojeni = false)
{
if ($spojeni === false):
  return mysql_list_fields($jmeno_databaze,$jmeno_tabulky);
else:
  return mysql_list_fields($jmeno_databaze,$jmeno_tabulky,$spojeni);
endif;
}

function phprs_sql_list_processes($spojeni = false)
{
if ($spojeni === false):
  return mysql_list_processes();
else:
  return mysql_list_processes($spojeni);
endif;
}

function phprs_sql_list_tables($jmeno_databaze,$spojeni = false)
{
if ($spojeni === false):
  return mysql_list_tables($jmeno_databaze);
else:
  return mysql_list_tables($jmeno_databaze,$spojeni);
endif;
}
...
function phprs_sql_tablename($vysledek,$cislo)
{
return mysql_tablename($vysledek,$cislo);
}

function phprs_sql_unbuffered_query($dotaz,$spojeni = false)
{
if ($spojeni === false):
  return mysql_unbuffered_query($dotaz);
else:
  return mysql_unbuffered_query($dotaz,$spojeni);
endif;
}
?>