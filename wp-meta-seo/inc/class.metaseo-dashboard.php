...
class MetaSeoDashboard
{
    ...
    public static function metaTitle()
    {
        ...
        $query      = 'SELECT DISTINCT ID '
                      ...
                      . ' INNER JOIN (SELECT * FROM ' . $wpdb->postmeta . ' WHERE meta_key = "_metaseo_metatitle" AND meta_value != "") mt ON mt.post_id = p.ID ';
        $query      .= ' WHERE ' . implode(' AND ', $where);
        // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Variable has been prepare
        $ps = $wpdb->get_col($query);

        $query2      = 'SELECT DISTINCT ID '
            . ' FROM ' . $wpdb->posts . ' as p'
            . ' INNER JOIN (SELECT * FROM ' . $wpdb->postmeta . ' WHERE meta_key = "wpms_validate_analysis" AND meta_value != "") mt ON mt.post_id = p.ID ';
        $query2      .= ' WHERE ' . implode(' AND ', $where);
        // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Variable has been prepare
        $ps2 = $wpdb->get_col($query2);

        $postIDs = array_merge($ps, $ps2);
        $metatitle_filled = count($postIDs);
        $results = array(
    ...
    public static function metaDesc()
    {
        ...
        $query   = 'SELECT DISTINCT ID '
            . ' FROM ' . $wpdb->posts . ' as p'
            . ' INNER JOIN (SELECT * FROM ' . $wpdb->postmeta . ' WHERE meta_key = "_metaseo_metadesc" AND meta_value != "") mt ON mt.post_id = p.ID ';
        $query   .= ' WHERE ' . implode(' AND ', $where);
        // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Variable has been prepare
        $wpms_posts      = $wpdb->get_col($query);
        ...
        $query   .= ' WHERE ' . implode(' AND ', $where);
        // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Variable has been prepare
        $wpms_posts2      = $wpdb->get_col($query);

        $postIDs = array_merge($wpms_posts, $wpms_posts2);
        $metadesc_filled = count($postIDs);
