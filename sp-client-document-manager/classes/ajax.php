...
public function new_password($vars){
	#print_r($vars);
foreach($vars as $key=>$var){
...
if(cdm_verify_recpatcha(array('token'=>cdm_var('token'), 'action'=>cdm_var('action'))) == false){
	...
	if($vars['password1'] == '' or $vars['password1'] == ''){
		echo '<div class="alert alert-danger"><strong>ERROR:</strong> Please fill in all fields!</div>';	
	}else{
		if($vars['password1'] == $vars['password2']){
			
		$user = get_user_by( 'email', $vars['email'] );	
		wp_set_password( $vars['password1'],$user->ID );
		wp_password_change_notification( $user );
		$login['username'] = $user->user_login;
		$login['password'] = $vars['password1'];
		$this->remind_user_email($user, $vars['password1']);
		$this->login($login);
		}else{
		echo '<div class="alert alert-danger"><strong>ERROR:</strong> Passwords do not match!</div>';		
		}
...
if(cdm_verify_recpatcha(array('token'=>cdm_var('token'), 'action'=>cdm_var('action'))) == false){
	...
	$user = get_user_by( 'email', $vars['email'] );
	if($user != false){
	$key = md5($vars['email'].'|'.md5(time()));
	set_transient( $key, $vars['email'],HOUR_IN_SECONDS );

	$message = 'Someone has requested a new password click here if you would like to continue to reset your password: '.cdm_shortcode_url('key='.$key.'').'';
  # $headers = 'From: My Name <myname@example.com>' . "\r\n";
   add_filter('wp_mail_content_type', 'set_html_content_type');
		wp_mail($vars['email'], 'Password Reset',apply_filters('sp_cdm/mail/content',stripslashes( $message)), $headers );	
	remove_filter('wp_mail_content_type', 'set_html_content_type');	
	echo '<div class="alert alert-success">Please check your email to reset your password!</div>';
	}else{
	echo '<div class="alert alert-danger"><strong>ERROR:</strong> Email Not Found</div>';	
	}
}
